...
class _DoctorDetailPageState extends State<DoctorDetailPage> {
  ...
  //select Date
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker( // async func that returns selected datetime object
      context: context,
      initialDate: DateTime.now(),// current
      firstDate: DateTime.now(), // makes sure i cant select date before today
      lastDate: DateTime(2101), // cant select after year 2101
    );
    if (picked != null && picked != _selectedDate) { // checks for null or previously selected
      setState(() {
        _selectedDate = picked;
      });
    }
  ...
  void _bookAppointment() {
    if (_selectedDate != null &&
      ...
      String time = _selectedTime!.format(context);
      String description = _descriptionController.text;
      String requestId = _requestDatabase.push().key!; // unique key generated by push from firebase
      String currentUserId = _auth.currentUser!.uid;
      String receiverId = widget.doctor.uid;
      String status = 'pending';
      ...
      _requestDatabase.child(requestId).set({
        ...
        setState(() {
          _selectedDate = null;
          _selectedTime = null;  // resets previous filled for new ui, cant display the info if user opens again
          _descriptionController.clear();
        });
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Appointment booked successfully')));
