...
import 'package:ongc_doctor_app/chat_screen.dart';
...
class _DoctorChatlistPageState extends State<DoctorChatlistPage> {
  ...
  List<Patient> _chatList = []; // A list of Patient objects that will be displayed in the chat list.
  ...
  Future<void> _fetchChatList() async {
    if(doctorId.isNotEmpty){
      try{
        final DatabaseEvent event = await _chatListDatabase.child(doctorId).once(); //Fetches the chat list from the Firebase ChatList node for the specific doctor (doctorId).
